...
#     print(lis_tag)
    ...
    if hangul.is_hangul(EF_next):
        return result
    #다로 끝나는 친구들
    if EF in dic.da_case1:
        result = result.replace('다'+EF_next, '입니다'+EF_next)
    elif EF in dic.da_case2:
        if EF == 'ㄴ다':
    ...
    elif EF in dic.da_case3:
        if '는다' in EF:
            result = result.replace('는다'+EF_next, '습니다'+EF_next)
        else:
            result = result.replace('다'+EF_next, '습니다'+EF_next)
    elif EF == '다':
        if lis_tag[EF_index-1] in dic.da_tag_case1:
            result = result.replace('다'+EF_next, '입니다'+EF_next)
        elif lis_tag[EF_index-1] in dic.da_tag_case2:
